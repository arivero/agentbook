...
**Version-controlled agent execution** is an emerging pattern that applies version control primitives to agent execution artifacts. Tools like ContextSubstrate (February 2026) implement content-addressed execution snapshots, treating agent runs as immutable, tamper-evident artifacts identified by cryptographic hashes. This enables git-like workflows for agent debugging: execution history (`ctx log`), snapshot inspection (`ctx show`), execution diffing (`ctx diff`), and best-effort replay (`ctx replay`).

The convergence with supply-chain provenance standards (SLSA, in-toto) is notable: execution substrates provide the same guarantees for agent runs that build systems provide for software artifacts. As agents move into production, expect version-controlled execution to become standard infrastructure, with federated execution registries enabling cross-organization audit trails and deterministic replay environments improving debugging fidelity.

The trend reflects a broader shift: agents are no longer ephemeral processes but long-lived systems requiring the same operational rigor (version control, auditability, reproducibility) as traditional software. For detailed technical coverage, see [Reproducible Execution Infrastructure](100-failure-modes-testing-fixes.md#3a-reproducible-execution-infrastructure).
