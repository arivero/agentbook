...
**Programmable environment evolution for benchmarks.** An emerging direction takes this further by making the test environment itself a variable. ProEvolve (arXiv:2603.05910) represents environments as typed relational graphs—data entities, tool operations, and schema definitions—and applies graph transformations to evolve them systematically. This shifts the benchmark question from "can the agent complete this task?" to "does the agent maintain performance as the environment changes?"—a more realistic measure of production readiness. As APIs evolve, tool schemas update, and data models change in real deployments, evaluation frameworks that test for adaptability rather than static correctness will become increasingly important. For scaffolding patterns that support this, see [Agentic Scaffolding](030-scaffolding.md#environment-evolution-infrastructure).
