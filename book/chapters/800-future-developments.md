...
Authority-pressure evaluations are joining prompt-injection and data-leakage suites as standard safety gates. Organisations are starting to track refusal rates and evidence-preservation scores when agents receive directives that conflict with policy, mirroring the patterns in [Failure Modes, Testing, and Fixes](100-failure-modes-testing-fixes.md#authority-driven-evidence-suppression). Expect certification regimes to require auditable dissent channels and immutable logging for high-impact agent actions.
