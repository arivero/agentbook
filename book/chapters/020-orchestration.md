...
**Challenge: Authority-Pressure Misalignment.** In compliance and oversight workflows, authority structures embedded in system prompts can cause agents to suppress accurate reporting in service of organisational directives—a failure mode empirically demonstrated across multiple current LLMs. The **solution** is to include independent review agents with neutral system prompts that check outputs for omissions, and to add explicit value hierarchy instructions stating that legal obligations and harm-reporting requirements take precedence over organisational directives. For test strategies and mitigation patterns, see [Common Failure Modes, Testing, and Fixes](100-failure-modes-testing-fixes.md#agentic-scheming-under-authority-pressure).
